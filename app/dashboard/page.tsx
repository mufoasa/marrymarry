...
async function getDashboardData() {
  ...
  if (profile.role === 'service_provider') {
    const [servicesRes, pendingServicesRes, inquiriesRes] = await Promise.all([
      supabase.from('services').select('*', { count: 'exact', head: true }).eq('owner_id', profile.id),
      supabase.from('services').select('*', { count: 'exact', head: true }).eq('owner_id', profile.id).eq('status', 'pending'),
      supabase.from('service_inquiries').select('id, service_id', { count: 'exact', head: true }),
    ]);

    stats = {
      totalVenues: servicesRes.count || 0,
      pendingReservations: pendingServicesRes.count || 0,
